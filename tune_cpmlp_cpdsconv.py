...
def build_command(args: argparse.Namespace) -> list[str]:
    ...
    if args.config_names:
        command.extend(["--config-names", args.config_names])
    if args.config_file:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Tune the current best CPMLP+CPDSConv fusion model under the "
            "condition-aware gap split."
        )
    )
    ...
    parser.add_argument("--output-root", default=script_dir / "tuning_outputs_cpmlp_cpdsconv_condition_gap")
...
def main() -> None:
    ...
    print("Running CPMLP+CPDSConv tuning command:")
    print(" ".join(command))
